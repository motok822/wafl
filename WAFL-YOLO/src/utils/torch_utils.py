...
def wafl_smart_resume(ckpts, optimizers, emas=None, weights='yolov5s.pt', epochs=300, resume=True, num_clients=10):
    # Resume training from a partially trained checkpoint
    best_fitnesses = []
    start_epoch = ckpts[0]['epoch'] + 1
    for i in range(num_clients):
        if ckpts[i]['optimizer'] is not None:
            optimizers[i].load_state_dict(ckpts[i]['optimizer'])  # optimizer
            best_fitness = ckpts[i]['best_fitness']
            best_fitnesses.append(best_fitness)
        if emas and ckpts[i].get('ema'):
            emas[i].ema.load_state_dict(ckpts[i]['ema'].float().state_dict())  # EMA
            emas[i].updates = ckpts[i]['updates']
    if resume:
        assert start_epoch > 0, f'{weights} training to {epochs} epochs is finished, nothing to resume.\n' \
                                f"Start a new training without --resume, i.e. 'python train.py --weights {weights}'"
        LOGGER.info(f'Resuming training from {weights} from epoch {start_epoch} to {epochs} total epochs')
    if epochs < start_epoch:
        LOGGER.info(f"{weights} has been trained for {ckpts[0]['epoch']} epochs. Fine-tuning for {epochs} more epochs.")
        epochs += ckpts[0]['epoch']  # finetune additional epochs
    return best_fitnesses, start_epoch, epochs
